(() => {
  ...
  const symbol = (cat, fixed=false) => {
    ...
    return `<span class="slot-symbol symbol-cat-id${fixed?' fixed-symbol':''}" style="--symbol-color:${color}">${pawFallback('slot-cat-fallback')}<img src="./${cat.imagePath}" alt="${cat.name}" onerror="this.hidden=true"><b>${cat.name}</b></span>`;
  };
  function slotVisual(rarity,result) {
    ...
    const hero=result.special?`<div class="slot-fever-hero" style="--cat-theme:${result.hero.themeColor};--cat-accent:${result.hero.accentColor}">${pawFallback('slot-hero-fallback')}<img src="./${result.hero.imagePath}" alt="${result.hero.name}" onerror="this.hidden=true"><b>${result.hero.name}</b></div>`:'';
