(() => {
  ...
  const CAT_LIFE_GOAL_PROGRESSION_AVAILABLE = CAT_LIFE_ENABLED && window.ChokinFeatureFlags?.catLifeGoalProgressionAvailable === true;
  const RELEASE_VERSION = window.ChokinRelease?.revision || 'r17';
  ...
  let catLifeViewModulePromise = null;
  let catLifeEventsRuntimePromise = null;
  let catLifeViewer = null;
  let backupV2RuntimePromise = null;
  let catLifeActivationDeferred = false;
  let catLifeActivationBusy = false;
  let financialActivationDeferred = false;
  let financialActivationBusy = false;
  let goalProgressionActivationDeferred = false;
  let goalProgressionActivationBusy = false;
  const $ = s => document.querySelector(s);
  const yen = n => `¥${Number(n || 0).toLocaleString('ja-JP')}`;
  const coinIcon = (kind='cat') => window.ChokinVisualAssets?.coinMarkup(kind) || '🪙';
  const pawFallback = className => window.ChokinVisualAssets?.pawMarkup?.(className) || `<span class="${className}" aria-hidden="true"></span>`;
  ...
  function loadCatLifeViewModule(){if(!CAT_LIFE_ENABLED)return Promise.resolve(null);if(!catLifeViewModulePromise)catLifeViewModulePromise=import('./cat-life-view.js?v=5').then(()=>window.ChokinCatLifeView).then(module=>{if(!module)throw new Error('猫たちの暮らし画面を準備できませんでした。');return module;}).catch(error=>{catLifeViewModulePromise=null;throw error;});return catLifeViewModulePromise;}
  ...
  async function loadFinancialRuntime() {
    ...
    if(!financialRuntimePromise)financialRuntimePromise=Promise.all([
      ...
      import('./cat-life-financial.js?v=1'),import('./cat-life-financial-transaction.js?v=4'),import('./cat-life-financial-runtime.js?v=2')
  ...
  async function loadGoalProgressionRuntime() {
    ...
    if(!goalProgressionRuntimePromise)goalProgressionRuntimePromise=Promise.all([
      ...
      import('./cat-life-goal-progression.js?v=1'),import('./cat-life-goal-progression-runtime.js?v=2')
    ]).then(([lifeRuntime])=>{
  ...
  async function runCatLifeOperation(callback,{recover=false,financial:financialOperation=false}={}) {
    ...
    return window.ChokinCatLifeCoordination.run(()=>{
      if(recover&&financial?.pending()){
        ...
        if(gacha.journalPresent||gacha.legacyPresent)throw new Error('複数の未完了保存があります。データを保持して停止しました。');
        financial.recover();
      }
  ...
  async function readCatLifeSnapshot() {
    const [runtime,financial,progression]=await Promise.all([loadCatLifeRuntime(),loadFinancialRuntime(),loadGoalProgressionRuntime()]);
    return window.ChokinCatLifeCoordination.run(()=>{
      const gacha=window.ChokinGachaTransaction.inspect();
      if(gacha.journalPresent||gacha.legacyPresent)throw new Error(gachaRecoveryMessage);
      return {runtime,loaded:runtime.loadRoot(new Date().toISOString()),collectionData:currentCollection(),eventLoadResult:financial.loadEvents(),progressionLoadResult:progression.loadState(),progressionActivationResult:progression.loadActivation()};
    });
  }
  async function processDailyCatLifeEvent(timestamp=new Date()){
    if(!CAT_LIFE_EVENTS_ENABLED)return {status:'disabled',committed:false,event:null};
    const before=financialActivationLoad(),financial=CAT_LIFE_FINANCIAL_AVAILABLE&&before.status==='ok'&&before.state.enabled===true;
    try{return await runCatLifeOperation(runtime=>{
      const activation=financialActivationLoad();
      if(!['empty','ok'].includes(activation.status))return {status:'financial_activation_safe_stop',committed:false,event:null};
      const enabled=CAT_LIFE_FINANCIAL_AVAILABLE&&activation.state.enabled===true;
  ...
  async function processGoalProgression(timestamp=new Date()){
    if(!CAT_LIFE_GOAL_PROGRESSION_AVAILABLE)return {status:'disabled',committed:false};
    try{const result=await runCatLifeOperation((financial,progression)=>{
      const financialActivation=financialActivationLoad();
      if(financialActivation.status!=='ok'||financialActivation.state.enabled!==true)return {status:'financial_not_activated',committed:false};
      if(!progression?.isEnabled())return {status:'not_activated',committed:false};
      return progression.settle({timestamp});
    },{recover:true,financial:true});showGoalProgressionSettlement(result.settlement);return result;}
  ...
  async function persistEntryState(nextState,timestamp=new Date()){
    if(!CAT_LIFE_GOAL_PROGRESSION_AVAILABLE){state=nextState;saveState();return {status:'legacy_saved',committed:true,mainState:state};}
    const result=await runCatLifeOperation((financial,progression)=>{
      ...
      const financialActivation=financialActivationLoad();
      if(financialActivation.status!=='ok'||financialActivation.state.enabled!==true){state=nextState;saveState();return {status:'financial_not_activated',committed:true,mainState:state};}
  ...
  async function persistMetadataState(nextState,timestamp=new Date()){
    if(!CAT_LIFE_GOAL_PROGRESSION_AVAILABLE){state=nextState;saveState();return;}
    await runCatLifeOperation((financial,progression)=>{
      const financialActivation=financialActivationLoad();
      const financialEnabled=financialActivation.status==='ok'&&financialActivation.state.enabled===true;
      if(financialEnabled&&progression?.isEnabled()){
  ...
  async function activateOwnedLegacyCatLife(timestamp=new Date().toISOString()){
    ...
    const runtime=await loadCatLifeRuntime();
    return runCatLifeOperation(()=>runtime.activateLegacy({collectionData:currentCollection(),mainState:currentMainForLife(),timestamp}));
  }
  const CAT_LIFE_ACTIVATION_FAILURE = '猫たちの暮らしを開始できませんでした。現在のデータは変更されていません。';
  function setCatLifeActivationStatus(message='',failed=false){const status=$('#catLifeActivationStatus');if(!status)return;status.textContent=message;status.classList.toggle('is-error',failed);}
  function setCatLifeActivationBusy(busy){catLifeActivationBusy=busy;['#startCatLife','#deferCatLife'].forEach(selector=>{const button=$(selector);if(button)button.disabled=busy;});}
  function hideCatLifeActivation(){const panel=$('#catLifeActivation');if(panel)panel.hidden=true;}
  async function inspectCatLifeActivation(timestamp=new Date().toISOString()){
    const panel=$('#catLifeActivation');
    if(!panel||!CAT_LIFE_ENABLED||catLifeActivationDeferred){hideCatLifeActivation();return {status:CAT_LIFE_ENABLED?'deferred':'disabled',missing:[]};}
    const collectionData=window.ChokinCollection.exportData(),records=collectionData?.cats&&typeof collectionData.cats==='object'?collectionData.cats:{},ownedIds=Object.entries(records).filter(([,record])=>record?.obtained===true).map(([catId])=>catId);
    if(!ownedIds.length){hideCatLifeActivation();return {status:'unowned',missing:[]};}
    try{
      const runtime=await loadCatLifeRuntime(),loaded=runtime.loadRoot(timestamp);
      if(!loaded.root){hideCatLifeActivation();return {status:'blocked',reason:loaded.status,missing:[]};}
      const knownIds=new Set(runtime.catWorld.cats.map(cat=>cat.id)),missing=ownedIds.filter(catId=>knownIds.has(catId)&&!loaded.root.cats[catId]);
      panel.hidden=missing.length===0;
      setCatLifeActivationStatus();
      return {status:missing.length?'eligible':'complete',missing};
    }catch(error){console.warn('猫たちの暮らしの開始条件を確認できませんでした。',error);hideCatLifeActivation();return {status:'blocked',reason:'runtime_error',missing:[]};}
  }
  function showCatLifeActivationSuccess(count){const toast=document.createElement('div');toast.className='cat-life-activation-toast';toast.setAttribute('role','status');toast.textContent=count>0?`${count}匹の猫たちの暮らしが始まりました。`:'猫たちの暮らしが始まりました。';document.body.append(toast);setTimeout(()=>toast.remove(),4200);}
  async function startCatLifeActivation(){
    if(catLifeActivationBusy)return;
    setCatLifeActivationBusy(true);setCatLifeActivationStatus();
    try{
      const result=await activateOwnedLegacyCatLife();
      if(result.committed===true){hideCatLifeActivation();showCatLifeActivationSuccess(result.created.length);void processDailyCatLifeEvent();return result;}
      if(result.status==='unchanged'){hideCatLifeActivation();return result;}
      console.warn('猫たちの暮らしを開始できませんでした。',result);setCatLifeActivationStatus(CAT_LIFE_ACTIVATION_FAILURE,true);return result;
    }catch(error){console.error('猫たちの暮らしを開始できませんでした。',error);setCatLifeActivationStatus(CAT_LIFE_ACTIVATION_FAILURE,true);return {status:'failed',committed:false,created:[],error};}
    finally{setCatLifeActivationBusy(false);}
  }
  function setupCatLifeActivation(){
    const panel=$('#catLifeActivation'),start=$('#startCatLife'),defer=$('#deferCatLife');
    if(!panel||!start||!defer)return;
    start.addEventListener('click',startCatLifeActivation);
    defer.addEventListener('click',()=>{if(catLifeActivationBusy)return;catLifeActivationDeferred=true;hideCatLifeActivation();});
    panel.addEventListener('keydown',event=>{if(event.key==='Escape'&&!catLifeActivationBusy){event.preventDefault();catLifeActivationDeferred=true;hideCatLifeActivation();}});
  }
  function setupCatLifeViewer(){
    const entry=$('#homeCatLifeView');
    if(!entry)return;
    if(!CAT_LIFE_ENABLED){entry.hidden=true;return;}
    entry.hidden=false;
    entry.addEventListener('click',async()=>{
      const status=$('#catLifeViewStatus');
      navigate('cat-life-view');
      if(status){status.textContent='猫たちの暮らしを読み込んでいます。';status.classList.remove('is-error');}
      try{
        const module=await loadCatLifeViewModule();
        if(!catLifeViewer)catLifeViewer=module.setup({screen:$('#cat-life-view'),list:$('#catLifeViewList'),detail:$('#catLifeViewDetail'),status:$('#catLifeViewStatus'),detailBack:$('#catLifeDetailBack'),loadRuntime:loadCatLifeRuntime,loadEvents:CAT_LIFE_EVENTS_ENABLED?loadCatLifeEventsRuntime:null,getCollection:()=>window.ChokinCollection.exportData(),getCatalog:()=>window.ChokinCats.all,readSnapshot:readCatLifeSnapshot,navigate});
        await catLifeViewer.open();
        refreshFinancialActivation();
        await refreshGoalProgressionActivation();
      }catch(error){console.error('猫たちの暮らしを表示できませんでした。',error);if(status){status.textContent='猫たちの暮らしを読み込めませんでした。現在のデータは変更されていません。';status.classList.add('is-error');}}
    });
  }
  function setFinancialActivationBusy(busy){financialActivationBusy=busy;['#startCatLifeFinancial','#deferCatLifeFinancial'].forEach(selector=>{const button=$(selector);if(button)button.disabled=busy;});}
  function setFinancialActivationStatus(message='',failed=false){const status=$('#catLifeFinancialActivationStatus');if(!status)return;status.textContent=message;status.classList.toggle('is-error',failed);}
  function refreshFinancialActivation(){
    const panel=$('#catLifeFinancialActivation');
    if(!panel)return {status:'missing'};
    if(!CAT_LIFE_FINANCIAL_AVAILABLE||financialActivationDeferred){panel.hidden=true;return {status:CAT_LIFE_FINANCIAL_AVAILABLE?'deferred':'unavailable'};}
    const loaded=financialActivationLoad();
    if(loaded.status==='ok'&&loaded.state.enabled===true){panel.hidden=true;return {status:'enabled',state:loaded.state};}
    panel.hidden=false;
    const safe=['empty','ok'].includes(loaded.status);
    setFinancialActivationBusy(!safe);
    setFinancialActivationStatus(safe?'':'開始設定を確認できません。データは変更されていません。',!safe);
    return loaded;
  }
  async function startFinancialActivation(){
    if(financialActivationBusy||!CAT_LIFE_FINANCIAL_AVAILABLE)return;
    setFinancialActivationBusy(true);setFinancialActivationStatus();
    try{
      const result=await runCatLifeOperation(()=>window.ChokinCatLifeFinancialActivation.activate({storage:localStorage,timestamp:new Date()}),{financial:true});
      if(result.committed===true||result.status==='already_enabled'){
        $('#catLifeFinancialActivation').hidden=true;
        const notice=document.createElement('p');notice.className='cat-life-activation-toast';notice.setAttribute('role','status');notice.textContent='猫たちのお金の暮らしを始めました。変化は明日以降のできごとから始まります。';document.body.append(notice);setTimeout(()=>notice.remove(),5200);
        void refreshGoalProgressionActivation();
        void processDailyCatLifeEvent();
        return result;
      }
      setFinancialActivationStatus('猫たちのお金の暮らしを開始できませんでした。データは変更されていません。',true);return result;
    }catch(error){console.error('猫たちのお金の暮らしを開始できませんでした。',error);setFinancialActivationStatus('猫たちのお金の暮らしを開始できませんでした。データは変更されていません。',true);return {status:'failed',committed:false,error};}
    finally{setFinancialActivationBusy(false);}
  }
  function setupFinancialActivation(){
    const panel=$('#catLifeFinancialActivation'),start=$('#startCatLifeFinancial'),defer=$('#deferCatLifeFinancial');
    if(!panel||!start||!defer)return;
    start.addEventListener('click',startFinancialActivation);
    defer.addEventListener('click',()=>{if(financialActivationBusy)return;financialActivationDeferred=true;panel.hidden=true;});
  }
  function setGoalProgressionActivationBusy(busy){goalProgressionActivationBusy=busy;['#startCatLifeGoalProgression','#deferCatLifeGoalProgression'].forEach(selector=>{const button=$(selector);if(button)button.disabled=busy;});}
  function setGoalProgressionActivationStatus(message='',failed=false){const status=$('#catLifeGoalProgressionActivationStatus');if(!status)return;status.textContent=message;status.classList.toggle('is-error',failed);}
  function showGoalProgressionSettlement(settlement){
    const amounts=Array.isArray(settlement?.credits)?settlement.credits.map(item=>item?.amount).filter(amount=>Number.isSafeInteger(amount)&&amount>0):[];
    if(!amounts.length)return;
    const notice=document.createElement('p');notice.className='cat-life-activation-toast';notice.setAttribute('role','status');
    notice.textContent=new Set(amounts).size===1?`貯金にあわせて、1匹につき＋${amounts[0].toLocaleString('ja-JP')}円`:'貯金にあわせて、猫たちの貯金も増えました。';
    document.body.append(notice);setTimeout(()=>notice.remove(),5200);
  }
  async function refreshGoalProgressionActivation(){
    const panel=$('#catLifeGoalProgressionActivation');
    if(!panel)return {status:'missing'};
    if(!CAT_LIFE_GOAL_PROGRESSION_AVAILABLE||goalProgressionActivationDeferred){panel.hidden=true;return {status:CAT_LIFE_GOAL_PROGRESSION_AVAILABLE?'deferred':'unavailable'};}
    try{
      const financial=financialActivationLoad();
      if(financial.status!=='ok'||financial.state.enabled!==true){panel.hidden=true;return {status:'financial_not_activated'};}
      const records=currentCollection()?.cats||{},ownedIds=Object.entries(records).filter(([,record])=>record?.obtained===true).map(([catId])=>catId);
      if(!ownedIds.length){panel.hidden=true;return {status:'unowned'};}
      const runtime=await loadGoalProgressionRuntime(),activation=runtime.loadActivation(),progressionState=runtime.loadState();
      if(activation.status==='ok'&&activation.state.enabled===true&&progressionState.status==='ok'){panel.hidden=true;return {status:'enabled',state:activation.state};}
      const safe=['empty','ok'].includes(activation.status)&&progressionState.status==='empty';
      panel.hidden=false;setGoalProgressionActivationBusy(!safe);setGoalProgressionActivationStatus(safe?'':'開始設定を確認できません。データは変更されていません。',!safe);
      return {status:safe?'eligible':'blocked'};
    }catch(error){panel.hidden=false;setGoalProgressionActivationBusy(true);setGoalProgressionActivationStatus('開始設定を確認できません。データは変更されていません。',true);return {status:'blocked',error};}
  }
  async function startGoalProgressionActivation(){
    if(goalProgressionActivationBusy||!CAT_LIFE_GOAL_PROGRESSION_AVAILABLE)return;
    setGoalProgressionActivationBusy(true);setGoalProgressionActivationStatus();
    try{
      const records=currentCollection()?.cats||{},ownedCatIds=Object.entries(records).filter(([,record])=>record?.obtained===true).map(([catId])=>catId);
      const result=await runCatLifeOperation((financial,progression)=>{
        const activation=financialActivationLoad(),financialEnabled=activation.status==='ok'&&activation.state.enabled===true;
        return progression.activate({timestamp:new Date(),financialEnabled,ownedCatIds});
      },{recover:true,financial:true});
      if(result.committed===true||result.status==='already_enabled'){
        $('#catLifeGoalProgressionActivation').hidden=true;
        const notice=document.createElement('p');notice.className='cat-life-activation-toast';notice.setAttribute('role','status');notice.textContent='猫たちの貯金の歩みを始めました。これからの貯金が次の週から反映されます。';document.body.append(notice);setTimeout(()=>notice.remove(),5200);
        if(catLifeViewer)await catLifeViewer.open();
        return result;
      }
      setGoalProgressionActivationStatus('猫たちの貯金の歩みを開始できませんでした。データは変更されていません。',true);return result;
    }catch(error){console.error('猫たちの貯金の歩みを開始できませんでした。',error);setGoalProgressionActivationStatus('猫たちの貯金の歩みを開始できませんでした。データは変更されていません。',true);return {status:'failed',committed:false,error};}
    finally{setGoalProgressionActivationBusy(false);}
  }
  function setupGoalProgressionActivation(){
    const panel=$('#catLifeGoalProgressionActivation'),start=$('#startCatLifeGoalProgression'),defer=$('#deferCatLifeGoalProgression');
    if(!panel||!start||!defer)return;
    start.addEventListener('click',startGoalProgressionActivation);
    defer.addEventListener('click',()=>{if(goalProgressionActivationBusy)return;goalProgressionActivationDeferred=true;panel.hidden=true;});
  }
  ...
  async function backupPayload() {
    ...
    return runCatLifeOperation((financial,progression)=>{
      ...
      if(eventLoaded.status==='empty'){
        const activation=financialActivationLoad();
        if(!['empty','ok'].includes(activation.status))throw new Error('猫たちのお金の開始設定を確認できません。');
        const enabled=CAT_LIFE_FINANCIAL_AVAILABLE&&activation.state.enabled===true;
        financial.processDay({enabled,activationDayKey:enabled?activation.state.activatedDayKey:null,timestamp:exportedAt,collectionData:currentCollection(),mainState:currentMainForLife()});
        eventLoaded=eventRuntime.loadState();
      }
      if(!['empty','ok'].includes(eventLoaded.status))throw new Error('猫たちのできごと履歴を確認できません。');
      const storedFinancial=financial.loadFinancial();
      if(!['empty','ok'].includes(storedFinancial.status))throw new Error('猫の暮らしの金融記録を確認できません。');
      const v1=backupV1Payload(exportedAt);
      // Capture live stored values, not another tab's stale in-memory module cache.
      const mainRaw=localStorage.getItem(KEY);
      if(mainRaw!==null){const main=normalizeMainBackupData(JSON.parse(mainRaw));if(!main)throw new Error('貯金記録を確認できません。');Object.assign(v1.data,main);}
      const fields={catCollection:window.ChokinCollection.key,catCoins:window.ChokinCoins.key,savingsGoal:window.ChokinSavingsGoal.getStorageKey(),goalHistory:window.ChokinGoalHistory.getStorageKey(),badgeState:window.ChokinBadges.getStorageKey(),dailyNotes:window.ChokinDailyNotes.getStorageKey()};
  ...
  function restoreStorageKeys(extraKeys=[]) {
    return [KEY,window.ChokinCollection.key,window.ChokinCoins.key,window.ChokinSavingsGoal.getStorageKey(),window.ChokinGoalHistory.getStorageKey(),window.ChokinBadges.getStorageKey(),window.ChokinDailyNotes.getStorageKey(),...extraKeys];
  }
  function captureRestoreSnapshot(extraKeys=[]) {
    return {state:structuredClone(state),collection:window.ChokinCollection.exportData(),coins:window.ChokinCoins.exportData(),goal:window.ChokinSavingsGoal.exportData(),history:window.ChokinGoalHistory.exportData(),badges:window.ChokinBadges.exportData(),dailyNotes:window.ChokinDailyNotes.exportData(),raw:new Map(restoreStorageKeys(extraKeys).map(key=>[key,localStorage.getItem(key)]))};
  }
  function restoreRawSnapshot(snapshot) {
    let restored=true;
    state=structuredClone(snapshot.state);
    const attempts=[()=>window.ChokinCollection.importData(snapshot.collection),()=>window.ChokinCoins.importData(snapshot.coins),()=>window.ChokinSavingsGoal.importData(snapshot.goal),()=>window.ChokinGoalHistory.importData(snapshot.history),()=>window.ChokinBadges.importData(snapshot.badges),()=>window.ChokinDailyNotes.importData(snapshot.dailyNotes)];
    attempts.forEach(attempt=>{try{attempt();}catch{restored=false;}});
    snapshot.raw.forEach((raw,key)=>{try{if(raw===null)localStorage.removeItem(key);else localStorage.setItem(key,raw);}catch{restored=false;}});
    try{render();}catch{restored=false;}
    return restored;
  }
  function verifyRestoredStorage(candidate,catLifeRuntime=null,eventRuntime=null) {
    ...
    if (!collectionRaw || JSON.stringify(collectionRaw.cats)!==JSON.stringify(collection.cats) || collectionRaw.totalCatMedals!==collection.totalCatMedals) throw new Error('猫図鑑を保存できませんでした。');
    const coinsRaw=JSON.parse(localStorage.getItem(window.ChokinCoins.key)||'null'),coins=window.ChokinCoins.exportData();
    ...
    try{
      ...
      return await runCatLifeOperation(()=>{
        ...
        if(v2){
          ...
          if(outcome.committed===true)return {ok:true,partial:false,reloadRequired:true};
          if(outcome.committed===null){document.querySelector('main').inert=true;return {ok:false,rollbackOk:true,message:'復元の保存状態を確認しました。再読み込みしてください。'};}
          return {ok:false,rollbackOk:true,message:'復元を確定できませんでした。開始前データを保持しています。'};
        }
        const transaction=backupRuntime.runSynchronous({capture:()=>captureRestoreSnapshot(extraKeys),apply:()=>applyRestoreData(candidate,v2?catLifeRuntime:null,v2?eventRuntime:null),verify:()=>verifyRestoredStorage(candidate,v2?catLifeRuntime:null,v2?eventRuntime:null),rollback:restoreRawSnapshot});
        ...
        return {ok:true,partial:!v2&&(!candidate.inspections.goal.valid||!candidate.inspections.history.valid||!candidate.inspections.badges.valid||candidate.inspections.collection.state==='partial'||candidate.inspections.coins.state==='partial'||candidate.inspections.dailyNotes.invalidItems>0)};
      },{financial:candidate?.has?.catLifeFinancialActivation&&candidate.inspections.catLifeFinancialActivation.state?.enabled===true});
    }catch(error){return {ok:false,rollbackOk:true,preflightBlocked:true,message:error instanceof Error?error.message:'復元準備に失敗しました。'};}
  }
  function finishRestore(result) {
    if(result.reloadRequired){alert('バックアップを復元しました。最新の保存状態で再起動します。');location.reload();return;}
    render();
    void inspectCatLifeActivation();
    alert(result.partial?'バックアップを復元しました。読み取れない任意項目は復元していません。':'バックアップを復元しました。');
    window.setTimeout(()=>window.ChokinBadges.evaluate(),0);
  }
  const load = () => {
  ...
  function renderCalendar() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      const dayEntries = groups.get(day) || [], records = recordGroups.get(day) || [], total = dayEntries.reduce((sum, entry) => sum + entry.amount, 0), isToday = calendarYear === today.getFullYear() && calendarMonth === today.getMonth() && day === today.getDate(), noteKey=window.ChokinDailyNotes.dateKeyFromParts(calendarYear,calendarMonth,day),hasNote=window.ChokinDailyNotes.hasNote(noteKey),canWriteNote=window.ChokinDailyNotes.isEnabled()&&noteKey<=window.ChokinDailyNotes.dateKey();
      if (records.length || hasNote || canWriteNote) {
        const saveLabel = dayEntries.length ? `、貯金 ${yen(total)}` : '',noteLabel=hasNote?'、ひとこと日記あり':'';
        cells.push(`<button class="calendar-day${records.length?' has-record':''}${dayEntries.length?` has-save tier-${calendarTier(total)}`:''}${isToday?' is-today':''}" type="button" role="gridcell" data-calendar-day="${day}" aria-label="${calendarYear}年${calendarMonth + 1}月${day}日${saveLabel}${records.length?`、記録${records.length}件`:''}${noteLabel}、詳細を開く"><time datetime="${noteKey}">${day}</time>${dayEntries.length?`<span class="calendar-paw" aria-hidden="true">${CALENDAR_PAW_SVG}</span><strong>${yen(total)}</strong>`:''}</button>`);
      } else {
  ...
  function openCalendarDay(day) {
    const entries = (calendarRecordGroups(calendarYear, calendarMonth).get(day) || []).map((entry,index)=>({entry,index,date:localDate(entry.createdAt)})).sort((a,b)=>a.date-b.date||a.index-b.index).map(item=>item.entry),noteKey=window.ChokinDailyNotes.dateKeyFromParts(calendarYear,calendarMonth,day);
    $('#calendarDetailTitle').textContent = `${calendarYear}年${calendarMonth + 1}月${day}日の記録`;
    $('#calendarDetailList').innerHTML = entries.length?entries.map(entry => { const date = localDate(entry.createdAt), time = date.toLocaleTimeString('ja-JP',{hour:'2-digit',minute:'2-digit'}), kind = entry.type === 'save' ? 'save' : entry.category, label = entry.type === 'save' ? '貯金' : categoryNames[entry.category], sign = entry.type === 'save' ? '＋' : '－'; return `<article class="calendar-detail-entry detail-${kind}"><div class="calendar-detail-main"><span class="calendar-detail-icon" aria-hidden="true">${CALENDAR_DETAIL_ICONS[kind]}</span><div><time datetime="${escapeHtml(entry.createdAt)}">${time}</time><b>${label}</b></div></div><strong>${sign}${yen(entry.amount)}</strong>${entry.memo ? `<p>${escapeHtml(entry.memo)}</p>` : ''}</article>`; }).join(''):'<p class="calendar-detail-empty">貯金・出費の記録はありません。</p>';
    ...
    window.ChokinDailyNotes.renderCalendarDetail(noteKey);
  ...
  function render() {
    ...
    renderCollection(); renderCoins(); window.ChokinDailyNotes.renderHome(); window.ChokinSavingsGoal.renderHome(); window.ChokinBadges.renderSettings(); if ($('#calendar')?.classList.contains('active')) renderCalendar(); if ($('#badges')?.classList.contains('active')) window.ChokinBadges.renderScreen();
  }
  ...
  function navigate(id) { window.ChokinSavingsGoal?.onNavigate?.(); window.ChokinGoalHistory?.onNavigate?.(id); document.querySelectorAll('.screen').forEach(x=>x.classList.toggle('active',x.id===id)); window.scrollTo(0,0); if(id==='calendar') showCurrentCalendarMonth(); if(id==='goal') window.ChokinSavingsGoal.renderDetail(); if(id==='goal-album') window.ChokinGoalHistory.renderAlbum(); if(id==='badges'){window.ChokinBadges.evaluate();window.ChokinBadges.renderScreen();} if(id==='history'||id==='collection') render(); }
  ...
  function buildSaveCheerCats(saveRank) {
    ...
    return selected.map((cat,index)=>`<span class="save-cheer-cat cheer-${index+1}" data-slot="${slots[index]}" data-cat-id="${cat.id}" data-rarity="${cat.rarity}" style="--cat-accent:${cat.accentColor};--cat-theme:${cat.themeColor}">${pawFallback('save-cheer-fallback')}<img src="./${cat.imagePath}" alt="" onerror="this.hidden=true"></span>`).join('');
  }
  function enhancedCelebrate(entry, forcedShow = null, preview = false, forcedRarity = null, options={}) {
    ...
    if(!box.contains(document.activeElement)&&document.activeElement!==document.body)celebrationReturnFocus=document.activeElement;
    ...
    const show=type==='save'?{name:gamePlan.visualShow||gamePlan.show,rare:gamePlan.rarity==='LEGEND'}:pick(expenseShows[type]);
    ...
    if(gachaResultMode){amountAnimationToken++;$('#eventAmount').textContent=focusCat?.name||'';}else animateEventAmount(entry.amount,reduced,type==='save');
    ...
    box.setAttribute('aria-hidden','false'); if(!options.silentStart){if(type==='save'){stageSound(rarity,confirmed,saveRank.level);stageHaptic(rarity,saveRank.level);}else{sound(type);haptic(type);}}
  }
  function showGachaFallback(plan,result){const cat=result.cat,progress=`猫図鑑 ${result.stats.obtained} / ${result.stats.total}匹`,nameSize=Math.max(18,Math.min(34,Math.floor(190/[...cat.name].length))),box=$('#celebration');box.className=`celebration show save gacha-result-mode rarity-${cat.rarity.toLowerCase()} ${result.isNew?'new-cat':'duplicate-cat'}`;$('#sceneVisual').innerHTML=`<div class="collection-reveal ${result.isNew?'new':'duplicate'}" style="--cat-accent:${cat.accentColor};--cat-theme:${cat.themeColor};--cat-name-size:${nameSize}px"><strong class="gacha-result-rarity">${cat.rarity}</strong><span class="collection-badge">${result.isNew?'NEW CAT！':'再会'}</span>${pawFallback('collection-fallback-result')}<img src="./${cat.imagePath}" alt="${escapeHtml(cat.name)}" onerror="this.hidden=true"><b>${escapeHtml(cat.name)}</b><small>${result.medals?`猫メダル ＋${result.medals}<br>`:''}${progress}</small></div>`;$('#eventType').textContent=cat.rarity;$('#eventMessage').textContent=result.isNew?'NEW CAT':'再会';$('#eventAmount').textContent=cat.name;$('#eventNote').textContent='';$('#revenge').hidden=true;box.setAttribute('aria-hidden','false');window.ChokinCanvasFX?.start($('#fxCanvas'),'gold','save',{reduced:true,rarity:cat.rarity});}
  function playCapsuleGacha(plan,result,preview=false){
    const box=$('#celebration'),reduced=!state.settings.effects||matchMedia('(prefers-reduced-motion: reduce)').matches,entry={type:'gacha',amount:0,category:null,memo:'',createdAt:new Date().toISOString()};
    if(!box.contains(document.activeElement)&&document.activeElement!==document.body)celebrationReturnFocus=document.activeElement;
    capsuleGachaSession?.cleanup();window.ChokinCanvasFX?.stop(true);window.ChokinAssets?.clear($('#sceneVisual'));previewActive=preview;
    box.className=`celebration show capsule-gacha-playing rarity-${plan.rarity.toLowerCase()}${reduced?' reduced':''}${preview?' preview-mode':''}`;box.setAttribute('aria-hidden','false');
    $('#sceneVisual').innerHTML='';$('#saveCheer').innerHTML='';$('#particles').innerHTML='';$('#eventType').textContent='';$('#eventMessage').textContent='';$('#eventAmount').textContent='';$('#eventNote').textContent='';$('#revenge').hidden=true;$('#closeEvent').textContent=preview?'設定へ戻る':'ホームへ';
    window.ChokinCanvasFX?.start($('#fxCanvas'),'cosmic','save',{reduced,rarity:plan.rarity});
    try{
      capsuleGachaSession=window.ChokinCapsuleGacha.play({host:$('#sceneVisual'),rarity:plan.rarity,reduced,onTap:()=>sound('save'),onPop:()=>{stageSound(plan.rarity,plan.confirmed,3);stageHaptic(plan.rarity,3);},onReveal:()=>{capsuleGachaSession=null;enhancedCelebrate(entry,null,preview,null,{gamePlan:plan,collectionResult:result,silentStart:true});}});
    ...
    try{
      ...
      const outcome=await runCatLifeOperation((financial,progression)=>{
        ...
        if(runtime&&collectionChange.result.isNew){
          ...
          const financialActivation=financialActivationLoad();
          const progressionEnabled=financialActivation.status==='ok'&&financialActivation.state.enabled===true&&progression?.isEnabled();
          if(CAT_LIFE_GOAL_PROGRESSION_AVAILABLE&&progressionEnabled){
