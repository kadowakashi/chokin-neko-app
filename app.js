(() => {
  ...
  const backupPayload = () => ({backupVersion: BACKUP_VERSION, exportedAt: new Date().toISOString(), appVersion: APP_VERSION, data: {version: state.version, entries: state.entries, settings: state.settings, futureSettings: state.futureSettings || {}, quickAmounts: state.quickAmounts, catCollection: window.ChokinCollection.exportData(), catCoins: window.ChokinCoins.exportData(), savingsGoal: window.ChokinSavingsGoal.exportData(), goalHistory: window.ChokinGoalHistory.exportData()}});
  ...
  function importBackup(file) {
    ...
    reader.onload = () => {
      try {
        ...
        saveState(); render(); alert(historyImported?'バックアップを読み込みました。':'達成アルバム以外のバックアップを読み込みました。達成アルバムの形式を確認してください。');
  ...
  const firstUse=localStorage.getItem(KEY)===null,welcomeCoinGranted=window.ChokinCoins.grantWelcome();load(); window.ChokinGoalHistory.setup({navigate}); window.ChokinSavingsGoal.setup({getEntries:()=>state.entries,getSettings:()=>state.settings,navigate}); setupQuickSettings(); setupEffectPreview(); setupCatGallery(); setupCollectionSettings(); setupCoinSettings(); setupPwaSupport(); render();scheduleCoinDayRefresh();if(firstUse&&localStorage.getItem(GUIDE_KEY)!=='seen')showFirstGuide();if(welcomeCoinGranted)setTimeout(showWelcomeCoin,500);
})();
