(() => {
  ...
  async function mount(container, showName, type) {
    const token = ++mountToken, sceneKey = keyFor(showName, type), entries = assets[sceneKey];
    if (!entries || !container) return false;
    if(entries.some(asset=>asset.mode==='main'))container.classList.add('generated-main-pending');
    try {
      ...
      for(const asset of entries){
        ...
        const imageProcessor=window.ChokinCatImages,requiresCleanBackground=asset.src==='assets/scenes/necessary_expense_stamp_cat.png',disallowLegacySource=(sceneKey==='temple'||sceneKey==='cosmic')&&asset.src.startsWith('assets/cats-transparent/'),canProcess=imageProcessor?.isProcessableSource?.(asset.src)||imageProcessor?.isCatSource?.(asset.src);
        ...
        if(asset.mode==='main'&&canProcess){
          const cleaned=await imageProcessor.processElement(image,asset.src,{allowSourceFallback:!disallowLegacySource});
          if(cleaned){URL.revokeObjectURL(url);objectUrls.delete(url);url=null;}
          else if(requiresCleanBackground||disallowLegacySource){URL.revokeObjectURL(url);objectUrls.delete(url);continue;}
        }
    ...
    finally { if(token===mountToken)container.classList.remove('generated-main-pending'); }
  }
  function clear(container) {
    mountToken++;
    container?.classList.remove('has-generated-main','has-generated-bg','generated-main-pending');
